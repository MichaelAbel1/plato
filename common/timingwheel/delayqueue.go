...
// 初始化一个 priorityQueue，容量为 capacity
...
// 比较函数  Priority 小的元素优先
func (pq priorityQueue) Less(i, j int) bool {
	return pq[i].Priority < pq[j].Priority
}

// 交换 pq[i] 和 pq[j]，并更新索引
...
type DelayQueue struct {
	C chan interface{} // 过期元素的输出通道

	mu sync.Mutex    // 互斥锁，保证并发安全
	pq priorityQueue // 优先级队列，存储待执行任务

	// Similar to the sleeping state of runtime.timers.
	sleeping int32         // 是否处于“睡眠”状态（即没有待执行任务）
	wakeupC  chan struct{} // 唤醒 Poll 线程的信号
}

// New creates an instance of delayQueue with the specified size.
// 创建 DelayQueue 并初始化 priorityQueue 和 wakeupC
...
func (dq *DelayQueue) Poll(exitC chan struct{}, nowF func() int64) {
	for {
		...
		dq.mu.Lock()
		item, delta := dq.pq.PeekAndShift(now) // 尝试取出最早到期的任务，如果没有任务到期则返回nil 并返回剩余等待时间
		if item == nil {
			...
			atomic.StoreInt32(&dq.sleeping, 1) // 如果没有到期任务，进入休眠
		}
		dq.mu.Unlock()

		if item == nil { // 等待任务到期或新任务插入
			if delta == 0 {
				// No items left.
				select {
				case <-dq.wakeupC:
				...
				case <-time.After(time.Duration(delta) * time.Millisecond):
					...
					if atomic.SwapInt32(&dq.sleeping, 0) == 0 { // atomic.SwapInt32返回旧值
